...
function superLevel(
  ...
  let ascore = 1 - Math.exp(-Math.sqrt(stats.totalFollowers + 1) / 50)
  // let bscore = 0
  ...
  let cscore = stats.totalCollects / stats.totalPosts +
    (stats.totalPosts * 0.5 + stats.totalMirrors * 0.3 + stats.totalComments * 0.2) + 1;
  cscore = 1 - Math.exp(-Math.sqrt(cscore) / 9)
  const count = ascore * cscore;
